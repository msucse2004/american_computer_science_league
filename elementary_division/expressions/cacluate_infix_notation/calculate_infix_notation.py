...
class CalculateInfixNotation(Expression):
    def __init__(self):
        Expression.__init__(self)
        self._title = "Calculate Infix Notation"
    ...
    def generate_problem(self) -> tuple[str, str]:
        ...
        self._operation_instance.rule_lower_upper_limit = (-3000, 3000)

        while True:
            infix_notation = self._operation_instance.generate_random_expression()
            evaluation_infix = self.evaluate_infix(infix_notation)
            if isinstance(evaluation_infix, int):
                problem_text = f"Calculate infix notation. [{infix_notation} =]"
                answer_text = str(evaluation_infix)
                break
...
def main():
    topic_instance = CalculateInfixNotation()
