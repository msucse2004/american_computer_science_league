...
class CalculatePostfixNotation(Expression):
    def __init__(self):
        super().__init__()
        self._title = "Calculate Postfix Notation"
    ...
    def generate_problem(self) -> tuple[str, str]:
        ...
        while True:
            ...
            evaluation_postfix = self.evaluate_postfix(postfix_notation)
            if isinstance(evaluation_postfix, int):
                print(f"{infix_notation} -> {postfix_notation} = {evaluation_postfix}")
                problem_text = f"Calculate postfix notation. [{postfix_notation} =]"
                answer_text = str(evaluation_postfix)
                break
...
def main():
    topic_instance = CalculatePostfixNotation()
