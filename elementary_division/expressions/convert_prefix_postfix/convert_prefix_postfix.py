...
class ConvertPrefixPostfix(Expression):
    def __init__(self):
        super().__init__()
        self._title = "ConvertPrefixPostfix"
    ...
    def generate_problem(self) -> tuple[str, str]:
        ...
        self._operation_instance.rule_lower_upper_limit = (-10000, 10000)


        infix_notation = self._operation_instance.generate_random_expression()
        prefix_notation = self.infix_to_prefix(infix_notation)
        postfix_notation = self.infix_to_postfix(infix_notation)

        problem_choice = random.choice(["postfix", "prefix"])

        if problem_choice == "postfix":
            problem_text = f"Convert postfix to prefix notation. [{postfix_notation}]"
            answer_text = prefix_notation
        else:
            problem_text = f"Convert prefix notation to postfix notation. [{prefix_notation}]"
            answer_text = postfix_notation
...
def main():
    topic_instance = ConvertPrefixPostfix()
