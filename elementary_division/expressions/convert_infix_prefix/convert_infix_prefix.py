...
class ConvertInfixPrefix(Expression):
    def __init__(self):
        super().__init__()
        self._title = "ConvertInfixPrefix"
    ...
    def generate_problem(self) -> tuple[str, str]:
        ...
        problem_choice = random.choice(["infix", "prefix"])

        if problem_choice == "infix":
            problem_text = f"Convert infix notation to prefix notation. [{infix_notation}]"
            answer_text = prefix_notation
        else:
            problem_text = f"Convert prefix notation to infix notation. [{prefix_notation}]"
...
def main():
    topic_instance = ConvertInfixPrefix()
