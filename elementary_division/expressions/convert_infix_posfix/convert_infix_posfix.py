...
class ConvertInfixPostfix(Expression):
    def __init__(self):
        super().__init__()
        self._title = "ConvertInfixPostfix"
    ...
    def generate_problem(self) -> tuple[str, str]:
        ...
        problem_choice = random.choice(["infix", "postfix"])

        if problem_choice == "infix":
            problem_text = f"Convert infix notation to postfix notation. [{infix_notation}]"
            answer_text = postfix_notation
        else:
            problem_text = f"Convert postfix notation to infix notation. [{postfix_notation}]"
...
def main():
    topic_instance = ConvertInfixPostfix()
